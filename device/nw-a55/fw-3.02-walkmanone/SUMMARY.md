...
| `/emmc@android` | `/system` | ext4 | `ro,noatime,nodiratime,data=ordered` |
...
| `/emmc@db` | `/db` | ext4 | `rw,nodev,noexec,noatime,nodiratime,discard,data=ordered` |
| `/emmc@option1` | `/opt1` | ext4 | `ro,nodev,noexec,noatime,data=ordered` |
| `/emmc@option3` | `/opt3` | ext4 | `ro,nodev,noexec,noatime,data=ordered` |
| `/emmc@option2` | `/opt2` | ext4 | `ro,nodev,noexec,relatime,discard,data=ordered` |
...
Sony libraries: `libAudioInPlayerService.so`, `libFuncArchUtils.so`, `libSoundService.so`, `libTunerPlayerService.so`, `libpstcore.so`
...
Sockets: `/tmp/bt.app.gap`, `/tmp/bt.ext.adp`, `@pst::services::bttransmitterservice`
...
Sony libraries: `libNcServiceFw.so`, `libpstcore.so`
...
Sony libraries: `libConnMgrService.so`, `libConnMgrServiceFw.so`, `libFuncArchUtils.so`, `libFuncMgrService.so`, `libPathMgrServiceFw.so`, `libTunerPlayerService.so`, `libVolumeGlue.so`, `libWiredHpService.so`, `libpstcore.so`
...
Sony libraries: `libConfigurationService.so`, `libFuncArchUtils.so`, `libPowerService.so`, `libappmgrservice.so`, `libpstcore.so`
...
Sony libraries: `DFGothicPW5-BIG5HK-SONY-20131004part.ttf`, `NotoSansKR-Bold.otf`, `NotoSansKR-Light.otf`, `SST-Bold.otf`, `SST-Light.otf`, `SST-Roman.otf`, `SSTUI-Light.ttf`, `SSTUI-Roman.ttf`, `libAudioAnalyzerService.so`, `libAudioAnalyzerServiceClient.so`, `libAudioInRecorderServiceClient.so`, `libBleFunctionHandler.so`, `libBtBleCommonService.so`, `libBtCommonService.so`, `libBtCompIf.so`, `libBtMw.so`, `libBtPlayerService.so`, `libBtTransmitterService.so`, `libConfigurationService.so`, `libConnMgrService.so`, `libConnMgrServiceFw.so`, `libDisplayService.so`, `libDmpConfig.so`, `libDmpFeature.so`, `libEffectCtrlDmp.so`, `libFuncArchUtils.so`, `libFuncMgrService.so`, `libFuncMgrServiceFw.so`, `libInitialObject.so`, `libKeyService.so`, `libLogupStub.so`, `libMediaStoreService.so`, `libMediaStoreServiceClient.so`, `libNcAsmService.so`, `libNcServiceFw.so`, `libNfcService.so`, `libOneTrackPlayerService.so`, `libOneTrackPlayerServiceClient.so`, `libPathMgrService.so`, `libPathMgrServiceFw.so`, `libPlayerService.so`, `libPlayerServiceClient.so`, `libPlayerServiceClientUtil.so`, `libPowerMgrServiceClient.so`, `libPowerMgrServiceFw.so`, `libQt5Core.so.5`, `libQt5Gui.so.5`, `libQt5Network.so.5`, `libQt5Qml.so.5`, `libQt5Quick.so.5`, `libQt5Widgets.so.5`, `libRecFileEditUtil.so`, `libRecorderServiceClient.so`, `libScreenshotShared.so`, `libSoundService.so`, `libSoundServiceFw.so`, `libSoundServiceSettingsDmp.so`, `libStorageMgrService.so`, `libStorageMgrServiceFw.so`, `libTimerService.so`, `libTunerPlayerService.so`, `libUsbDeviceAudioPlayerService.so`, `libUsbMgrService.so`, `libUsbMgrServiceFw.so`, `libVolumeService.so`, `libVolumeServiceFw.so`, `libWiredHpService.so`, `libappmgrservice.so`, `libaudiohal-a2dpsnksingletrack.so`, `libaudiohal-adleralsa.so`, `libaudiohal-analyzer.so`, `libaudiohal-dualtrackmixalsa.so`, `libaudiohal-genericalsa.so`, `libaudiohal-listener.so`, `libaudiohal-uacalsasingletrack.so`, `libcnpysort.so`, `libcrypto.so.1.1`, `libcurl.so.4`, `libeaselcore.so`, `libeaselqt.so`, `libharfbuzz.so.0`, `libicudata.so.61`, `libicui18n.so.61`, `libicuuc.so.61`, `libjson.so`, `libpstcore.so`, `libssl.so.1.1`
...
| `bootanimation` | `/system/xbin/icx_bootanimation` |
| `console` | `/bin/sh -I` |
...
| `icx_syslog` | `/system/xbin/icx_syslog -l 6 -d "/emmc@var"` |
...
`icx_usbconn`, `icx_carddet`, `wm_key`, `mtk_stp_bt_soc`, `mtk_stp_wmt_soc`, `mtk_wmt_detect`, `icx_nvp_emmc`
